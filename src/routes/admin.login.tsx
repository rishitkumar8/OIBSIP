...
export const Route = createFileRoute("/admin/login")({
  head: () => ({ meta: [{ title: "Admin sign-in — Forno" }] }),
...
function AdminLogin() {
  ...
  return (
    <div className="relative flex min-h-screen items-center justify-center bg-gradient-to-br from-secondary/30 via-background to-primary/10 p-4">
      {/* Floating pizza emojis backdrop */}
      ...
      <Card className="relative w-full max-w-md border-primary/30 shadow-2xl">
        <CardHeader className="text-center">
          ...
          <p className="text-sm text-muted-foreground">For pizzeria staff only.</p>
        </CardHeader>
        <CardContent>
          ...
          <div className="mt-4 flex items-center justify-between text-sm">
            <Link to="/admin/signup" className="text-primary hover:underline">Create admin account</Link>
            <Link to="/auth" className="text-muted-foreground hover:underline">User sign-in</Link>
          </div>
        </CardContent>
      </Card>
    </div>
